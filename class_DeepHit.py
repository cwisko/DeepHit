...
FC_Net = _layers.fully_connected
...
class Model_DeepHit:
    def __init__(self, sess, name, input_dims, network_settings):
        ...
        self.reg_W              = tf.keras.regularizers.l2(l=1e-4)
    ...
    def _build_net(self):
        with tf.compat.v1.variable_scope(self.name):
            ...
            self.rate   = tf.compat.v1.placeholder(tf.float32, [], name='rate')   #keeping rate
            ...
            shared_out = create_FCNet(self.x, self.num_layers_shared, self.h_dim_shared, self.active_fn, self.h_dim_shared, self.active_fn, self.initial_W, self.rate, self.reg_W)
            ...
            for _ in range(self.num_Event):
                cs_out = create_FCNet(h, (self.num_layers_CS), self.h_dim_CS, self.active_fn, self.h_dim_CS, self.active_fn, self.initial_W, self.rate, self.reg_W)
                out.append(cs_out)
    ...
    def loss_Log_Likelihood(self):
        ...
        tmp1 = tf.math.reduce_sum(tf.math.reduce_sum(self.fc_mask1 * self.out, axis=2), axis=1, keepdims=True)
        tmp1 = I_1 * log(tmp1)

        #for censored: log \sum P(T>t|x)
        tmp2 = tf.math.reduce_sum(tf.math.reduce_sum(self.fc_mask1 * self.out, axis=2), axis=1, keepdims=True)
        tmp2 = (1. - I_1) * log(tmp2)

        self.LOSS_1 = - tf.math.reduce_mean(tmp1 + 1.0*tmp2)
    ...
    def get_cost(self, DATA, MASK, PARAMETERS, rate, lr_train):
        (x_mb, k_mb, t_mb) = DATA
        (m1_mb, m2_mb) = MASK
        ...
        return self.sess.run(self.LOSS_TOTAL, 
                             feed_dict={self.x:x_mb, self.k:k_mb, self.t:t_mb, self.fc_mask1: m1_mb, self.fc_mask2:m2_mb, 
                                        ...
                                        self.mb_size: np.shape(x_mb)[0], self.rate:rate, self.lr_rate:lr_train})

    def train(self, DATA, MASK, PARAMETERS, rate, lr_train):
        (x_mb, k_mb, t_mb) = DATA
        (m1_mb, m2_mb) = MASK
        (alpha, beta, gamma) = PARAMETERS
    ...
    def predict(self, x_test, rate=1.0):
        return self.sess.run(self.out, feed_dict={self.x: x_test, self.mb_size: np.shape(x_test)[0], self.rate: rate})
